...
The [implementation registry](implementations.json) declares package paths, build, adapter, package test, and runtime commands. `make check` runs each declared package test command; shared cases exercise the JSON contract and cannot reach a language-specific API, so an API that only one package provides requires tests in that package. [package-tests.json](package-tests.json) lists the case every implementation runs, each exemption with its reason, and each package's own cases; `make check` compares that standard with the cases the suites report and fails on a missing or undeclared case. Add new languages there and in a package directory without changing the shared JSON comparison algorithm. A contract change updates the verifier and affected packages in one repository revision. See the [repository contract](docs/spec/repositories.md).
