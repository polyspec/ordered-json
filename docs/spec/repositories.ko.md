<!-- doc-id: repositories -->
<!-- source-sha256: 96d8793f35ba6fb84a88ffc395d1373a6636ef5136ef087a776ee664e4bb127b -->
# 저장소 계약
...
공식 입력과 기대 결과는 공통 저장소에만 존재합니다. 어댑터는 공통 보고 형식을 반환하며 독립 기대값을 포함하지 않습니다. 구현 등록 정보는 저장소 URL을 한 번 선언하고, 저장소 위치, 어댑터 명령, 빌드 명령, 패키지 테스트 명령, 런타임 버전 명령을 정의합니다. 통합 검사는 선언된 패키지 테스트 명령을 모두 실행하고 결과를 기록하며, 선언된 패키지 테스트가 빠진 기록은 거부합니다. 공통 사례는 어댑터를 통해 JSON 계약을 검사하며 언어 전용 API에는 닿지 않으므로, 그 API는 해당 패키지의 테스트가 담당합니다. 패키지 테스트 표준은 모든 구현이 실행하는 케이스, 사유를 적은 면제, 패키지 고유 케이스, 그리고 공개 심볼마다 그것을 검증하는 케이스를 정의합니다. 각 구현은 자신이 실행하는 케이스를 보고하는 명령과 공개 심볼을 보고하는 명령을 선언하고, 통합 검사가 두 보고를 표준과 대조합니다. 검증하는 케이스가 없는 공개 심볼과 더 이상 내보내지 않는 심볼 선언은 모두 검사를 실패시킵니다. 언어 추가에 JSON 비교 알고리즘 변경이 필요해서는 안 됩니다.

검증 기록이 담는 모든 값에는 비교 대상이 있어야 합니다. 저장소의 현재 상태이거나, 저장소가 선언한 기대값입니다. 소스와 패키지 해시는 작업 트리와 비교합니다. 저장소 밖에서 오는 입력, 곧 PIE 도구와 추가 입력 모음은 [외부 입력](../../external-inputs.json)에 고정하며, 이를 명시한 기록은 고정값과 일치해야 합니다. 둘 중 어느 것과도 비교할 수 없는 값은 근거가 아니므로 기록하지 않습니다. 링크된 모듈은 매번 새 식별자와 서명을 가지므로 어떤 재빌드도 같은 해시를 만들지 못합니다. 기록은 등록 정보가 선언한 산출물을 경로로만 명시합니다.
