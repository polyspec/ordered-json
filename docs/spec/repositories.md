...
Official inputs and expected results exist only in the common repository. Adapters return the shared reporting protocol and contain no independent goldens. The implementation registry declares the repository URL once, along with repository locations, adapter commands, build commands, package test commands, and runtime version commands. The aggregate check runs every declared package test command and records its result; a record that omits a declared package test is rejected. Shared cases verify the JSON contract through adapters and do not reach language-specific APIs, which their own package tests cover. The package test standard lists the cases every implementation runs, the exemptions with their reasons, each package's own cases, and the cases that cover each public symbol. Every implementation declares one command that reports the cases it runs and one that reports its public symbols, and the aggregate check compares both reports with the standard. A public symbol without a covering case, and a declared symbol the package no longer exports, both fail the check. Adding a language must not require changing the JSON comparison algorithm.

Every value a verification record carries must have something to compare it with: the repository's current state, or an expected value the repository declares. Source and package hashes are compared with the working tree. Inputs that come from outside, the PIE tool and the supplementary suite, are pinned in [external inputs](../../external-inputs.json), and a record that names one must match its pin. A value with neither comparison is not evidence and is not recorded. A linked module carries a fresh identifier and signature, so no rebuild reproduces its hash; the record names the artifact the registry declares by path alone.
