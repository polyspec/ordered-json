...
check('values come only from parse and factories', () => {
...
check('returned collections cannot be modified in place', () => {
...
check('wrong-kind access names both kinds', () => {
...
check('member lookup requires a string key', () => {
...
check('parse validates its arguments', () => {
...
check('parseBytes validates its input', () => {
...
check('parse errors report UTF-16 offsets', () => {
...
check('string units expose UTF-16 code units', () => {
...
check('factories validate their arguments', () => {
